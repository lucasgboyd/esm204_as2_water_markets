...
tax_df_drought <- template_df %>% 
  mutate(former_cost = tax_df$total_cost) %>% 
  mutate(abatement = tax/mca_slope) %>% # calculating abatement by each district by plugging in the tax and solving for quantity abated for each district
  mutate(total_cost = (((abatement)*(abatement)*mca_slope)/2) - former_cost)  %>% # calculating total cost by finding the area under the curve for each district at that level of abatement
  mutate(stringency = abatement/use) %>% # calculating stringency by dividing abatement by initial use 
...
mojave_policy <- round(tax_df$total_cost[2] + tax_df_drought$total_cost[2], 0)
antelope_policy <- round(tax_df$total_cost[3] + tax_df_drought$total_cost[3], 0)
...
-   **Mojave** prefers a **single tax** ($`r paste(mojave_policy)`)
 
-   **Antelope** prefers a **single tax** ($`r paste(antelope_policy)`)
